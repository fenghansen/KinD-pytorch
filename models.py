...
class DecomNet(nn.Module):
    def __init__(self, filters=32, activation='lrelu'):
        ...
        self.deconv_r1 = ConvTranspose2D(filters*2, filters)
        self.concat_r2 = Concat()
        self.conv_r3 = nn.Conv2d(filters, 3, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        deconv_r2 = self.deconv_r1(concat_r1)
        concat_r2 = self.concat_r1((conv_input, deconv_r2))
        conv_r3 = self.conv_r3(concat_r2)
        R_out = self.R_out(conv_r3)
        
        # build Illumination map
        conv_i1 = self.conv_i1(conv_input)
        concat_i1 = self.concat_i1((conv_r3, conv_i1))
