...
def gradient(maps, direction, device='cuda'):
    min_batch = maps.size()[0]
    channels = maps.size()[1]
    out_channel = channels
    smooth_kernel_x = Robert.expand(out_channel, channels, 2, 2)
    ...
    gradient_orig = torch.abs(F.conv2d(maps, weight=kernel, padding=(1, 0)))
...
class Decom_Loss(nn.Module):
    ...
    def forward(self, R_low, R_high, I_low, I_high, L_low, L_high):
        ...
        i_input_mutual_loss = self.illumination_smoothness(I_low, L_low) + self.illumination_smoothness(I_high, L_high) 
